...
import ChannelJson from './ChannleJson';
...
class ChannelForm extends React.Component {
  ...
  handleSrcChange(event) {
    ...
    this.state.channelJson.setName(value);
    this.state.channelJson.setSource(this.state.sourcePrefix + value); 
  }
  ...
  handleCrdChange(event) {
    ...
    const value = event.target.value;
    if (name === 'x') {
      this.state.channelJson.setCoordinate(value, 0);
    }
    else if (name === 'y') {
      this.state.channelJson.setCoordinate(value, 1);
    }
    else if (name === 'z') {
      this.state.channelJson.setCoordinate(value, 2);
    }
  ...
  handleSubmit(event) {
    ...
    var jsonText = JSON.stringify(this.state.channelJson);
